...
import { getClient } from '../lib/suiUtils';
...
const RPC = "https://sui-mainnet.blastapi.io/607c818d-3131-4be5-8752-29d30cbaffc3";

    
export async function getDepositWallets() {
  const supabase = createClient<Database>(SUPABASE_URL, SUPABASE_ANON_KEY);
  const { data: boosts, error } = await supabase.from('Boosts').select().eq("chain_id", 4);
 ...
 async function main() {
  ...
  boosts?.forEach(async boost => {
    ...
    const balance = await getBalance(boost.wallet_public_key ?? "", client)
    if(balance > 0) {
